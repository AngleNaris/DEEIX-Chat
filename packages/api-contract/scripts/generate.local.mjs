...
function generateSwagger(outputDir) {
  // 本机无 Go 工具链：委托 deeix-build 容器执行 `go tool swag init`。
  const hostOutput = outputDir.replaceAll("\\", "/");
  runCommand(
    "docker",
    [
      "run", "--rm",
      ...
      "deeix-build:1",
      "sh", "-c",
      "go tool swag init -g cmd/server/main.go -o /host-out --packageName docs --parseDependency --parseInternal --requiredByDefault --quiet",
    ],
    backendDir,
  );
}
