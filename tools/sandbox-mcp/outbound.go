// 本文件拷贝自 backend/internal/shared/security/outbound.go（canonical 源）。
//
// 沙箱 MCP 是独立 Go module（tools/sandbox-mcp），无法 import backend 的 internal 包，
// 故按审查结论 P0-03 拷贝此处复用出站安全策略：sandbox_download 的网络请求
// 必须拒绝私网/回环/link-local/metadata 目标，并对每次 DNS 解析与每次重定向复验。
//
// 来源：backend/internal/shared/security/outbound.go @ f18c9be5（custom 分支）。
// 上游 canonical 源更新安全规则时，应同步此文件（此文件保持与源文件一致的公开 API）。
package main
