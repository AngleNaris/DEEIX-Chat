...
- On Linux, published output is copied through scoped directory descriptors and atomically renamed into `/shared/deeix-<user>-<conversation>/mm-<call>/...`. Qwen response strings are rewritten to published paths and JSON-RPC `result.__export__` is added for the existing attachment pipeline. Invalid responses discard the just-published directory.
