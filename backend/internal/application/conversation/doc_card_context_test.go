...
func TestMatchDocCards(t *testing.T) {
	...
	matched := matchDocCards("我在魔法森林遇到Alice", cards, 0)
	if len(matched) != 2 {
	...
	// 禁用卡片不参与匹配。
	matched = matchDocCards("禁用词", cards, 0)
	if len(matched) != 0 {
		t.Fatalf("disabled card must not match, got %+v", matched)
	}
	// 上限。
	matched = matchDocCards("魔法森林 Alice", cards, 1)
	if len(matched) != 1 {
		t.Fatalf("expected cap 1, got %d", len(matched))
	}
	// 空查询/空列表。
	if matchDocCards("", cards, 0) != nil || matchDocCards("x", nil, 0) != nil {
